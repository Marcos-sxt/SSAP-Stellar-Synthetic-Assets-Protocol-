...
async function testChainlink() {
  ...
  try {
    ...
    console.log('📡 Test 1: Testing Chainlink connection...');
    const isConnected = await chainlink.testConnection();
    console.log(`Connection status: ${isConnected ? '✅ SUCCESS' : '❌ FAILED'}\n`);
    
    if (!isConnected) {
      console.log('❌ Chainlink connection failed. Stopping tests.');
      return;
    }
    ...
    for (const commodity of commodities) {
      try {
        const price = await chainlink.getPrice(commodity);
    ...
    try {
      const prices = await chainlink.getMultiplePrices(commodities);
